...
summary: "本指南为社区投稿提供了一套风格指南规范。"
type: "book"
weight: 1
---

本指南为云原生社区博客风格指南。主要目的为：
...
文件头部配置为 Hugo 静态网站编译所必须的，所有投稿文章都需要在投稿文件的头部添加文件头（文件头前后使用三个短横线“-”），例如：
...
- `sumnary`: 表示对投稿内容的简要描述，尽量控制在 100 字以内，例如：`本指南为技术文档的编写提供了一套风格指南规范。`。
- `authors`: 表示投稿作者，数组。
- `translators`：若是翻译的文章，则需要填写，若为原创则不需要。
- `categories`: 是用于分类技术类别的，例如 `Kubernetes`、`Istio` 等，建议只填一个值。
- `tags`: 表示投稿的 TAG，使用数组的形式，例如：`["社区", "风格", "投稿"]`。
- `links`：这一部分字段仅翻译的文章需要提供原文 `url`，若原创文章请删除 `links` 部分。

注意：以上为添加博客时可以指定 header，请参考已有文章。

## 图片

图片使用 Markdown 格式，使用本地图片，与文档 `index.md` 放在同一目录，图片格式为：

```text
![图片标题](图片路径）
```

支持的图片格式为 `svg`、`png`、`jpg`、`webp`。建议使用 [sqoop](https://sqoosh.app/) 对图片进行压缩。
...
连接号的常见形式为“—”，占一个字的位置。连接号还有另外四种形式，即长横“——”（占两个字的位置）、半字线“–”（占半个字的位置）、短划线“-”（占 1/3 个字的位置）和浪纹“~”（占一个字的位置）。
